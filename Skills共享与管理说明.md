...
```text
D:\My Files\Zxp_global_skills\skills
```
...
这个文件夹才是一个 Skill
↓
复制整个这个文件夹到 D:\My Files\Zxp_global_skills\skills
```

正确结构示例：

```text
D:\My Files\Zxp_global_skills\skills\paper-workflow\SKILL.md
D:\My Files\Zxp_global_skills\skills\figure-planner\SKILL.md
...
```text
D:\My Files\Zxp_global_skills\skills\某个GitHub仓库-main\README.md
D:\My Files\Zxp_global_skills\skills\某个GitHub仓库-main\images
```
...
```text
D:\My Files\Zxp_global_skills\github_sources
```
...
```text
D:\My Files\Zxp_global_skills\skillhub.config.json
```

记录 GitHub 仓库清单、哪些是 Skill、哪些只是 prompt、哪些仓库需要特殊安装规则。

```text
D:\My Files\Zxp_global_skills\SkillHub.ps1
```
...
```text
D:\My Files\Zxp_global_skills\更新GitHub来源并同步Skills.ps1
```

给自己日常使用的简短入口，本质上是调用 `SkillHub.ps1`。

```text
D:\My Files\Zxp_global_skills\SkillHub.UI.ps1
```
...
```text
D:\My Files\Zxp_global_skills\卸载每日自动更新任务.ps1
```

用于取消 Windows 每日自动更新任务。

```text
D:\My Files\Zxp_global_skills\reports\last-sync.md
```
...
```powershell
powershell -ExecutionPolicy Bypass -File "D:\My Files\Zxp_global_skills\更新GitHub来源并同步Skills.ps1"
```
...
```powershell
schtasks.exe /Query /TN ZxpGlobalSkillsDailyUpdate /V /FO LIST
```
...
```powershell
powershell -ExecutionPolicy Bypass -File "D:\My Files\Zxp_global_skills\安装每日自动更新任务.ps1"
```
...
```text
任务名：ZxpGlobalSkillsDailyUpdate
下次运行：每天 09:00
启动器：D:\ZxpSkillHubLauncher\run-skillhub.cmd
```
...
```powershell
powershell -ExecutionPolicy Bypass -File "D:\My Files\Zxp_global_skills\卸载每日自动更新任务.ps1"
```
...
```powershell
powershell -ExecutionPolicy Bypass -File "D:\My Files\Zxp_global_skills\SkillHub.ps1"
```
...
```text
1. 把整个 Zxp_global_skills 文件夹复制过去
...
如果复制到的路径不是 `D:\My Files\Zxp_global_skills`，也没有关系。
...
```text
D:\My Files\Zxp_global_skills\github_sources\impeccable\.claude\skills\impeccable
```
