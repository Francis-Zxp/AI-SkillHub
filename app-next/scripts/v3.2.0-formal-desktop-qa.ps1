...
$qaRoot = [IO.Path]::GetFullPath((Join-Path $env:TEMP "AI-SkillHub-v3.2.2-formal-desktop-qa-$qaRunId"))
...
'@
  ...
  $qaResult = [pscustomobject]@{
    ...
    StartupReport = (Join-Path $appNextRoot "reports\desktop\v3.2.2-startup-cache\formal-isolated-cache-$qaRunId.json")
    DragReport = (Join-Path $appNextRoot "reports\desktop\v3.2.2-tauri-drag\state-$qaRunId.json")
  }
} catch {
  $operationError = $_
} finally {
