...
const reportDir = path.resolve(__dirname, "../reports/visual/v3.2.5-external-skills");
...
const labels = {
  zh: { title: "本机 Skills", scan: "重新扫描", search: "搜索名称、说明或路径", all: "全部工具", status: "全部状态", read: "查看原文", add: "纳入技能库", close: "关闭原文", use: "使用此路径", keep: "保留原草稿" },
  en: { title: "Skills on this computer", scan: "Rescan", search: "Search name, description or path", all: "All tools", status: "All states", read: "Read original", add: "Add to library", close: "Close original", use: "Use this path", keep: "Keep existing draft" },
  ko: { title: "이 컴퓨터의 Skills", scan: "다시 검색", search: "이름, 설명 또는 경로 검색", all: "모든 도구", status: "모든 상태", read: "원문 보기", add: "라이브러리에 추가", close: "원문 닫기", use: "이 경로 사용", keep: "기존 초안 유지" }
};
...
async function main() {
  ...
  try {
    for (const lang of ["zh", "en", "ko"]) {
      ...
      assert.equal(await managed.getByRole("button", { name: label.add, exact: true }).isDisabled(), true);
      ...
      results.push({ language: lang, checks: ["scan", "search", "agent-filter", "removed-agent-filter-reset", "managed-filter", "safe-text-preview", "read-retry", "scan-retry", "project-context", "narrow-layout", "import-prefill"], overflow, ipcCalls: calls.length });
      await page.close();
    }
    ...
    console.log(`v3.2.5 external Skills QA passed: ${results.length} scenarios; ${reportDir}`);
