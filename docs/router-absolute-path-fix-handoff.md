...
> **3.1.12 复核结论（同日更新）**：本文的 914 条引用是 3.1.11 首次修复后的历史基线。
> 独立内容审计发现其中 20 条是名称和文件内容均完全相同的宿主打包副本，并非 20 个独立能力。
> 3.1.12 改为“同名 + 内容完全相同则合并；同名但内容不同则保留并标注宿主变体”，
> 真实路由现为 43 个父入口、894 条可打开声明，Claude、`.agents`、旧 `.codex` 和
> Antigravity 均为 894/894。下文“同名项一个也不合并”的说法仅记录原提交 `76772b2`，
> 已被 3.1.12 决策替代。
