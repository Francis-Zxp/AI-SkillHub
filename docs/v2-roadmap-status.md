...
| Tauri/React/Rust/SQLite code | `app-next` scaffold created; frontend build, Rust tests, read-only v1 scanner, SQLite indexing, SQLite-first loading, adapter registry, SQLite-only state toggles, selectable workspace detail UI, project scan detail, snapshot UI, backup target inventory UI, backup dry-run UI, restore dry-run UI, release gate UI, browser preview fallback, first UI density pass, release/build guide UI, desktop-window QA checklist UI, and first real Tauri desktop-window QA pass | 72% |
...
| Release readiness | first release gate page exists; it summarizes diagnostics, backup dry-run, restore dry-run, rollback lock, desktop QA, and share validation, but does not execute packaging | 42% |
...
Weighted together, the honest number is about **67%**.
...
2. Connect v1 share validation and release preflight report data into the v2 Release Gate as read-only inputs.
