...
V2 overall completion is conservatively estimated at **about 28%**.

This does **not** mean a usable V2 application is 28% coded. It means v1 is now stable enough to become the maintenance line, the first v2 `app-next` scaffold exists, the toolchain is ready, dependencies are installed, both frontend and Rust checks pass, and the first read-only scanner now reads real v1 Skills, sources, agents, and diagnostics.
...
| Tauri/React/Rust/SQLite code | `app-next` scaffold created; frontend build, Tauri info, Rust tests, and read-only v1 scanner pass | 25% |
...
Weighted together, the honest number is about **28%**.
...
5. The first read-only scanner can read existing AI SkillHub data and show it in the React/Tauri shell.

## Next Practical Step

Continue v2 milestone 1:

1. Persist the scanned result into SQLite without changing any v1 data.
2. Add a small scanner report panel so the user can see scan time, counts, and source of truth.
3. Run the first Tauri dev window after SQLite persistence is in place.
