...
- Keeps deep Windows repositories updateable by enabling repository-scoped Git long paths before checkout, and clearly distinguishes Git clones from non-updateable GitHub snapshots.
- Retries the exact Git for Windows Schannel credential failure with repository-scoped OpenSSL without changing global Git settings.
- Validates complete Prompt repositories through GitHub's committed tree metadata before materializing blobs, avoiding the partial-clone `ls-tree -l` stall while preserving file-count, depth, per-file, and total-byte limits.
- Makes source deletion explicitly recoverable and adds a one-click Git retry before a reviewed snapshot is promoted.
