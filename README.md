...
把分散在 GitHub、本地目录和不同 AI 工具里的 Skills，整理成一张可搜索、可评分、可路由、可同步的能力图谱。

AI SkillHub 是一款面向 Windows 的本地优先 Skill 管理器：导入真实 `SKILL.md`，区分 Prompt 资料与可调用 Skill，自动组织父子关系与同名路由，并把经过筛选的能力安全同步给 Claude Code、Codex、Antigravity 等工具。
...
- 自动建立父 Skill、子 Skill、同名冲突路由和来源限定别名。
- 给来源与子 Skill 进行本地 1–5 星评分，并按评分优先排序。
- 用真实来源、分类、父子关系和 GitHub 热度生成可交互能力星图。
- 检测 Claude Code、Codex、Antigravity 等本机工具，并以受控方式同步 Skills。
...
v3.0.3 是自动更新起点。首次安装 v3.0.3 后，软件会自动检查官方正式版；只有通过 AI SkillHub 更新公钥验证的签名安装包才会被接受。后续更新替换受管理的程序文件并重启，来源、评分、设置与 SQLite 索引继续保存在：

```text
%LOCALAPPDATA%\AI SkillHub\UserData\
```

v3.0.2 及更早版本没有更新器，因此需要手动安装一次 v3.0.3。旧便携目录不会被程序擅自递归删除；确认新版和资料库正常后再清理旧目录。

## 最短使用流程

1. 打开 AI SkillHub，进入「技能库」。
2. 粘贴 GitHub 仓库地址，选择 Skill / Prompt 类型与分类。
3. 点击「一键添加并刷新」。
4. 在来源卡片查看父 Skill、子 Skill、健康状态与评分。
